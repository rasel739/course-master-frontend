...
const EnrollmentDetails = () => {
  ...
  useEffect(() => {
    if (enrollment) {
      ...
      for (const mod of sortedModules) {
        ...
        for (const lesson of sortedLessons) {
          const isCompleted = enrollment.completedLessons.some(
            (cl) => cl.moduleId === mod._id && cl.lessonId === lesson._id
          );
          if (!isCompleted) {
            setCurrentLesson({ moduleId: mod._id, lessonId: lesson._id, lesson, module: mod });
            return;
          }
        }
      }
      ...
      if (sortedModules.length > 0 && sortedModules[0].lessons.length > 0) {
        const modules = sortedModules[0];
        const lesson = [...modules.lessons].sort((a, b) => a.order - b.order)[0];
        setCurrentLesson({ moduleId: modules._id, lessonId: lesson._id, lesson, module: modules });
      }
    }
  }, [enrollment]);
  ...
  const handleLessonSelect = (moduleId: string, lessonId: string) => {
    ...
    const course = enrollment.course as Course;
    const selectModule = course.modules.find((m) => m._id === moduleId);
    const lesson = selectModule?.lessons.find((l) => l._id === lessonId);
    if (selectModule && lesson) {
      setCurrentLesson({ moduleId, lessonId, lesson, module: selectModule });
    }
  };
  ...
  const goToNextLesson = () => {
    ...
    for (const nextModule of sortedModules) {
      const sortedLessons = [...nextModule.lessons].sort((a, b) => a.order - b.order);
      for (const lesson of sortedLessons) {
        if (foundCurrent) {
          setCurrentLesson({
        ...
        if (nextModule._id === currentLesson.moduleId && lesson._id === currentLesson.lessonId) {
          foundCurrent = true;
        }
      }
    }
                        ...
                        <CardContent className='p-6'>
                          ...
                          {loadingExtras ? (
                            ...
                            <div className='space-y-4'>
                              {currentModuleAssignments.map(
                                (assignment: Assignment & { hasSubmitted?: boolean }) => (
                                  <div
                                    key={assignment._id}
        ...
        <AssignmentModal
          assignment={
            selectedAssignment as Assignment & {
              ...
              userSubmission?: { content: string; submissionType: string } | null;
            }
          }
