...
import { ArrowLeft, BookOpen, CheckCircle, Clock, Calendar } from 'lucide-react';
...
export default function AdminStudentDetailPage() {
  ...
  const enrollment = enrollments.find((e) => e._id === params.id) || null;
  ...
  const remainingLessons = Math.max(0, totalLessons - (completedLessons?.length || 0));
      ...
      <Card>
        ...
        <CardContent className='p-0'>
          {completedLessons.length === 0 ? (
            <div className='p-12 text-center text-gray-500'>No lessons completed yet</div>
          ) : (
            <div className='divide-y'>
              {completedLessons.map((cl, index) => {
                const modules = course?.modules?.find((m) => m._id === cl.moduleId);
                const lesson = modules?.lessons?.find((l) => l._id === cl.lessonId);
